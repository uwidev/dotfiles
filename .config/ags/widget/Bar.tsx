...
function Workspaces(props) {
    ...
    return <box className="Workspaces">
        {bind(hypr, "workspaces").as(wss => wss
            // this filter will ONLY WORK if monitors are of different models...
            .filter(ws => ws.get_monitor().get_model() == props.monitor)
            .sort((a, b) => a.id - b.id)
            .map(ws => (
                <button
                    className={bind(hypr, "focusedWorkspace").as(fw =>
                        ws === fw ? "focused wbutton" : "wbutton")}
                    onClicked={() => ws.focus()}>
                    {ws.id}
                </button>
            ))
...
function Test() {
    return <button
        onClickRelease={(_, event) => console.log(event.button)}>
        {"Click me!"}
    </button>
}
